...
export const SECTIONS = [
  ...
  },
  {
    id: 'ai',
    title: 'AI 导学助手',
    subtitle: '做题遇到困难时的 AI 伙伴——审题、纠错、总结、推荐相似题，支持 @ 引用和 / 指令',
    sticker: 4
  },
  {
    id: 'qa',
    title: '课件问答',
    subtitle: '从课件 PDF 里直接检索答案、查页码、对原文——学概念时的专属助手',
    sticker: 1
  },
  {
    id: 'context',
    title: '共享能力：@ 引用与 / 指令',
    subtitle: 'AI 导学和课件问答都支持的上下文引用和快捷指令',
    sticker: 2
  },
  {
    id: 'flow',
  ...
  },
  {
    id: 'faq',
    title: '常见问题',
    subtitle: '判题卡住、课件问答没回应、AI 卡片不出现……',
    sticker: 5
  },
  {
    id: 'tour',
...
export const LEARNING_LOOP_STEPS = [
  ...
  },
  {
    step: 4,
    title: '写代码',
    desc: '把伪代码翻译成真代码；先写能跑的版本，不追求一次最优。完全没思路时，让 AI 出一份骨架代码，自己填关键空；如果骨架还是写不下去，点骨架代码卡片底部"有点难？试试拼装版"，把骨架打散成块按正确顺序拼回来。'
  },
  {
    step: 5,
    title: '提交',
...
export const AI_CAPABILITIES = [
  ...
  },
  {
    id: 'skeleton-fill',
    title: '生成骨架代码',
    desc: '给一份只剩关键逻辑要你自己填的代码骨架；点卡片底部"插入编辑器"直接带到编辑器里继续写。'
  },
  {
    id: 'parsons-fallback',
    title: '骨架太难时切到拼装版',
    desc: '骨架代码卡片底部"有点难？试试拼装版"按钮：把骨架打散成可拖拽块，按正确顺序拼回来——同样不直接给答案，但难度更低。'
  },
  {
    id: 'locate-bug',
    title: '定位代码错误',
  ...
  },
  {
    id: 'summarize-kc',
    title: '总结知识点',
    desc: '通过一题后用一两句话告诉你"这题考的是什么"，并引用课件相关章节帮你回顾。'
  },
  {
...
export const CONTEXT_TOKENS = [
  ...
  },
  {
    token: '@page:<章>.<页>',
    name: '课件中具体的某一页',
    when: '在 @ 菜单里按"第 X 章 · 文件名"二级目录展开当前课件，挑一页插入；token 形如 @page:1.7（第 1 章第 7 页），AI 会读这一页的原文回答。'
  },
  {
    token: '@courseware:<lpId>',
    name: '当前课件整包（RAG 检索）',
    when: '想让 AI 基于整份课件回答时使用；后端会用你这条问题做 RAG，top-k 段落塞进 LLM。@ 菜单只暴露当前题目所属的课件包，不会串到别的课程。'
  }
]
...
export const CONTEXT_EXAMPLES = [
  ...
  },
  {
    id: 'ctx-page',
    label: '@page:&lt;章&gt;.&lt;页&gt;',
    prompt: '@page:1.7 这一页讲的「变量赋值」是什么意思？我看不懂第二个例子。',
    note: '在 @ 菜单里按"第 X 章 · 文件名"二级目录挑一页插入；后端按当前课件包内 PDF 的 sort_order 序号定位章，再用 page_no 取这一页正文塞给 AI。比 @courseware 整包检索更精准。'
  },
  {
    id: 'ctx-courseware',
    label: '@courseware:&lt;lpId&gt;',
    prompt: '@courseware:42 课件里讲的「列表推导式」是什么意思？给两个对比例子说明用法。',
    note: '让 AI 基于整份课件原文答；后端会用你这条问题对该课件做 RAG，top-k 段落塞进 LLM，回答会附「（参考课件第 X 页）」标注。'
  }
]
...
export const CONTEXT_TIPS = [
  ...
  '@page 走二级目录：在 @ 菜单里先看到"第 X 章 · 文件名"，再展开挑具体页；token 写成 `@page:1.7` 表示第 1 章第 7 页，章号按当前课件包内 PDF 的 sort_order 1-based 序号取。',
  '@ 菜单里的课件 / 课件页只显示当前题目所属的课程包，不会串到别的课件，避免引用错位。',
...
export const COURSEWARE_QA_PROMPTS = [
  ...
  },
  {
    id: 'qa-page',
    label: '直接 @ 某一页追问',
    prompt: '@page:1.7 这一页的代码示例我没看懂，能用更简单的话解释一下吗？',
    note: '在 @ 菜单里按"第 X 章 · 文件名"二级目录挑一页，token 形如 @page:1.7（第 1 章第 7 页）；后端只取这一页的正文塞给 AI，比让它去整份课件里搜更精准。/page 1.7 也能直接跳到这一页的 PDF 预览。'
  }
]
...
export const COURSEWARE_QA_NOTES = [
  ...
  '@ 菜单里的课件页按"第 X 章 · 文件名"二级目录展开，token 形如 @page:1.7；只显示当前选中的课件包，不会串到别的课程。',
  '/page 同时接受 `/page 1.7` 与 `/page 7`：前者直接跳到第 1 章第 7 页 PDF，后者按全包连续页号定位。',
  '每次回答末尾 AI 会抛出一个思考题——建议先自己想一想，再继续对话深入探讨。',
  '课件问答也支持日常闲聊，AI 会自然引导回编程学习话题。'
]
...
export const FAQ_ITEMS = [
  ...
  {
    q: 'AI 回答后面的思考题有什么用？',
    a: '每个回答末尾的问题是 AI 针对你刚学的内容设计的引导思考题。先自己想 30 秒，想不出来再继续问——这个过程比直接看答案的学习效果好很多。'
  },
  {
    q: '课件问答里怎么看 PDF 原文？',
    a: '回答中引用的页码是可以点击的蓝色链接。点击后右侧会打开证据侧栏，直接显示 PDF 对应页面的内容。也可以点「在新标签页查看」全屏阅读。'
  },
  {
    q: '知识点回顾卡里的"课件参考"从哪来？',
    a: 'AI 导学会自动检索和当前题目知识点相关的课件页面。如果你看到"可回顾第 X 章 第 Y 页"，说明课件里有对应内容可以对照学习。'
  },
  {
    q: '/ 指令和 @ 引用有什么区别？',
    a: '@ 引用是把已有内容塞进 AI 对话（比如 @last_error 让 AI 看到你的错误）。/ 指令是触发一个动作（比如 /skeleton 生成骨架代码、/clear 清空对话）。两者可以组合使用。'
  },
  {
    q: '"拼装挑战"按钮在哪里？',
    a: '不再放在输入栏快捷按钮里。现在是骨架代码卡片底部的"有点难？试试拼装版"次级按钮——只有先让 AI 出一份骨架代码、卡片出现后才能看到。这样设计是因为拼装版本就是骨架代码的"再降一档"形态，把入口收到一起更顺手。'
  },
  {
    q: '@ 菜单里为什么只看到当前题目的课件？',
    a: '故意的。AI 导学和课件问答的 @ 菜单都已经按"当前题目 / 当前会话所属课件包"过滤，避免你引用到别的课程的页码导致 AI 答错。如果你想引用别的课件，去对应课件的题目或者直接在课件问答页里切到那个课程包。'
  },
  {
    q: '@page:1.7 是什么意思？怎么写对？',
    a: '"第 1 章 · 第 7 页"——章号是当前课件包内 PDF 按上传顺序（sort_order）排出来的 1-based 序号，每份 PDF 算一章；页号是这份 PDF 内的页码。最简单的方式是输入 @ 后从二级目录挑一页，菜单会自动把 token 拼好。手写时小心别多空格、别用全角点号。'
  },
  {
    q: '手机上能用吗？',
    a: '可以。页面做了响应式适配，手机浏览器直接访问即可。代码编辑体验建议在电脑上操作，但查看题目、看 AI 回答、错题复习在手机上都没问题。'
  }
]
