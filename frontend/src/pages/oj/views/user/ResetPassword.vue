    ...
    <div class="card-wrap">
      ...
      <div class="brand-panel">
        ...
        <div class="brand-top">
          ...
          <div class="brand-headline">Set new<br><span class="accent">password.</span></div>
          <div class="brand-desc">设置新的密码后，你可以立刻返回登录继续使用系统。</div>
        </div>
        <div class="brand-tags">
          <div class="brand-tag tag-judge">Secure</div>
          <div class="brand-tag tag-track">Strong</div>
          <div class="brand-tag tag-improve">Done</div>
        </div>
      </div>
      ...
      <div class="form-panel">
        ...
        <div class="form-body" v-if="!resetSuccess">
          ...
          <button class="login-btn rip"
            :class="{loading: btnLoading, success: resetSuccess}"
            @click="resetPassword"
            :disabled="btnLoading">
            <div class="btn-inner">
        ...
        <div class="form-body success-body" v-else>
          ...
          <h3 class="success-title">{{ $t('m.Your_password_has_been_reset') }}</h3>
          <p class="success-desc">密码已更新，请使用新密码登录。</p>
          <button class="login-btn success-green" @click="$router.push({name: 'login'})">
            <div class="btn-inner">
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        captchaSrc: '',
        formReset: { password: '', passwordAgain: '', captcha: '', token: '' },
        pwVisible: false,
        pw2Visible: false,
        btnLoading: false,
        resetSuccess: false,
    ...
    methods: {
      ...
      resetPassword () {
        ...
        if (!f.password) { this.showFieldErr('pass', '请输入新密码'); return }
        if (f.password.length < 6 || f.password.length > 20) { this.showFieldErr('pass', '密码长度 6-20 位'); return }
        ...
        api.resetPassword(data).then(res => {
          ...
          setTimeout(() => this.showToast('请使用新密码登录', 'warn'), 1200)
        }, _ => {
          this.btnLoading = false
          this.btnText = '重置密码'
          this.formReset.captcha = ''
          this.getCaptchaSrc()
          this.showToast('重置失败，请重试', 'err')
        })
      }
    }
  }
...
.success-green { background: linear-gradient(135deg, #34a853, #2bbd4e); box-shadow: 0 4px 16px rgba(52,168,83,.35); }
