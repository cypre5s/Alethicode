    ...
    <div class="card-wrap">
      <div class="brand-panel">
        ...
        <div class="brand-top">
          ...
          <div class="brand-headline">Create your<br><span class="accent">account.</span></div>
          <div class="brand-desc">注册后即可进入系统，开始刷题、评测与个性化学习。</div>
        </div>
      ...
      <div class="form-panel">
        <div class="form-top">
          ...
          <div class="form-top-title">Sign up</div>
          <div class="step-dots">
        ...
        <div class="form-body">
          <div class="field">
            ...
            <div class="field-wrap">
              ...
              <input class="f-input" ref="inputUser"
                ...
                v-model="formRegister.username" type="text"
          ...
          <div class="field">
            ...
            <div class="field-wrap">
              ...
              <input class="f-input" ref="inputEmail"
                :class="{error: fieldErr.email}"
                v-model="formRegister.email" type="email"
          ...
          <div class="field">
            ...
            <div class="field-wrap">
              ...
              <input class="f-input" ref="inputPass"
                ...
                @keydown.enter="handleRegister">
              <div class="pw-toggle" @click="pwVisible = !pwVisible">
          ...
          <div class="field">
            ...
            <div class="field-wrap">
              ...
              <input class="f-input" ref="inputPassAgain"
                :class="{error: fieldErr.passAgain}"
                v-model="formRegister.passwordAgain"
                :type="pw2Visible ? 'text' : 'password'"
                :placeholder="$t('m.Password_Again')"
                @input="clearFieldErr('passAgain')"
                @keydown.enter="handleRegister">
              <div class="pw-toggle" @click="pw2Visible = !pw2Visible">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" v-if="!pw2Visible"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" v-else><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
              </div>
              <div class="field-underline"></div>
            </div>
            <div class="field-error" :class="{show: fieldErr.passAgain}">
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
              <span>{{ fieldErr.passAgainMsg }}</span>
            </div>
          </div>
            ...
            <div class="captcha-row">
              <div class="field-wrap captcha-input-wrap">
                ...
                <input class="f-input"
                  ...
                  v-model="formRegister.captcha" type="text"
          ...
          <button class="login-btn rip"
            :class="{loading: btnLoading, success: regSuccess}"
            @click="handleRegister"
            :disabled="btnLoading || regSuccess">
            <div class="btn-inner">
              <div class="btn-spinner"></div>
              <svg v-if="regSuccess" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              <span class="btn-text">{{ btnText }}</span>
            </div>
          </button>

          <div class="register-row">
            已有账号？<span class="register-link" @click="goLogin">{{ $t('m.Already_Registed') }}</span>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    name: 'StandaloneRegister',
    data () {
      return {
        captchaSrc: '',
        formRegister: { username: '', email: '', password: '', passwordAgain: '', captcha: '' },
        pwVisible: false,
        pw2Visible: false,
        btnLoading: false,
        regSuccess: false,
    ...
    methods: {
      ...
      handleRegister () {
        const f = this.formRegister
        if (!f.username.trim()) { this.showFieldErr('user', '请输入用户名'); return }
        if (!f.email.trim()) { this.showFieldErr('email', '请输入邮箱'); return }
        if (!/\S+@\S+\.\S+/.test(f.email)) { this.showFieldErr('email', '邮箱格式不正确'); return }
        if (!f.password) { this.showFieldErr('pass', '请输入密码'); return }
        if (f.password.length < 6) { this.showFieldErr('pass', '密码至少 6 位'); return }
        if (f.password !== f.passwordAgain) { this.showFieldErr('passAgain', '两次密码不一致'); return }
        if (!f.captcha.trim()) { this.showFieldErr('captcha', '请输入验证码'); return }

        this.btnLoading = true
        this.btnText = '注册中...'

        let formData = Object.assign({}, f)
        delete formData.passwordAgain

        api.register(formData).then(res => {
          this.btnLoading = false
          this.regSuccess = true
          this.btnText = '注册成功，跳转中...'
          this.showToast('注册成功！')
          setTimeout(() => this.showToast('正在跳转到登录页...', 'warn'), 1200)
          setTimeout(() => this.goLogin(), 2500)
        }, _ => {
          this.btnLoading = false
          this.btnText = '注册'
          this.getCaptchaSrc()
          this.formRegister.captcha = ''
          this.showToast('注册失败，请检查信息', 'err')
        })
      },
...
.form-top {
  ...
  padding: 24px 36px 20px; position: relative; overflow: hidden;
...
.form-top-title { font-size: 24px; font-weight: 800; color: #fff; letter-spacing: -.3px; margin-bottom: 12px; }
...
.form-body {
  flex: 1; padding: 20px 36px 24px; display: flex; flex-direction: column;
  opacity: 0; animation: fadein .4s .45s ease forwards; overflow-y: auto;
}
...
.field-error {
  font-size: 11px; color: #ea4335; margin-top: 4px;
...
.captcha-img img { height: 42px; display: block; }
...
.register-row { text-align: center; margin-top: 16px; font-size: 12px; color: #9aa0ab; }
