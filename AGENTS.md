...
注释解释"为什么"，代码说明"做什么"。优先通过清晰命名消除注释需求。

### 必须注释

- 所有 `public` Java 方法用 Javadoc：首句摘要 + `@param` / `@return` /
  `@throws`，中文描述。
- 所有 Python 导出函数和类用 `"""三引号"""`：首行祈使句 + `Args` / `Returns`。
- 所有 JavaScript 导出函数用 `/** */`：`@param` / `@returns`。
- 非显见的设计取舍、边界约束、性能陷阱、workaround 用行内注释。
- 正则表达式必须注释其匹配意图。
- SQL 迁移文件首部必须说明变更目的和受影响的表。

### 禁止注释

- 叙述代码行为：`// 递增计数器`、`// 返回结果`、`// 导入模块`。
- 重复函数签名已说明的信息（参数名、类型）。
- 已被代码取代的历史注释；过时 TODO（应清理或建 issue）。
- 用注释替代版本控制的变更记录。

### 格式要求

- Java Javadoc 首句以句号结尾；多行时空行分隔描述与标签。
- Python docstring 首行祈使句（"压缩历史消息" 而非 "此函数压缩历史消息"）。
- Vue 组件通过 `name` / `emits` / `props` 自文档化，不另写注释。
- 行内注释与代码之间空两格；独立行注释与上方代码空一行。
