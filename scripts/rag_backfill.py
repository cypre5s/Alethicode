...
调用方式：
  # 估算成本（仅扫总数）
  python scripts/rag_backfill.py --estimate
  # 取最旧 100 条做尺寸校准
  python scripts/rag_backfill.py --limit 100
  # 全量
  python scripts/rag_backfill.py --all
  # 仅某类
  python scripts/rag_backfill.py --all --entity-types courseware-page

  # 重试历史失败行
  python scripts/rag_backfill.py --retry-errors

  # 清空进度（开发期 reset）
  python scripts/rag_backfill.py --reset
...
async def estimate_only(pool: asyncpg.Pool) -> None:
    queries = {
