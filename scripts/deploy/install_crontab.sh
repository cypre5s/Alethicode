...
BACKUP_JOB="0 3 * * * $ROOT_DIR/scripts/backup/auto_backup.sh >> $ROOT_DIR/db_backups/backup.log 2>&1"
...
if [[ -n "$DOMAIN" ]]; then
  RENEW_JOB="0 4 * * 1 $ROOT_DIR/scripts/deploy/certbot_renew.sh $DOMAIN >> $ROOT_DIR/db_backups/certbot.log 2>&1"
