...
用法:
  python nfk/run_local.py --quick          # 快速验证 (1 fold × 1 seed)
  python nfk/run_local.py --full           # 完整消融 (5 fold × 3 seed)
  python nfk/run_local.py --dataset ednet  # EdNet 数据集
"""
...
from nfk.data.preprocessor import (
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="assistments",
                        choices=["assistments", "ednet", "alethicode"])
    parser.add_argument("--data-path", type=str, default=None,
                        help="Override data path (for alethicode: path to NFK CSV exported by backend)")
    ...
    elif args.dataset == "alethicode":
        if not args.data_path:
            raise SystemExit(
                "--dataset alethicode requires --data-path pointing to the NFK CSV exported by backend"
            )
        data_path = args.data_path
        preprocessor = AlethicodeCsvPreprocessor(data_path)
    else:
