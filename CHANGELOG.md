...
- 2026-05-06 **[新增/服务]** `com.alethicode.service.career.studio` 包：`MicroProjectStudioService` 接口扩展为 5 方法（`recommendForUser` / `generate` / `listForUser` / `findById` / `markCompleted`）+ `MicroProjectStudioServiceImpl`（critic 通过后 → 提取 reference_solution.code + test_cases → 调 `LanguagePackProblemJudgeCheckService.executeReferenceSolution` 走 Judge Server 跑 reference 100% AC 自身 test_cases → 通过才落 `problem` 表 + `career_micro_project` 含 `judge_problem_id` → 失败 abort 不落库）+ `MicroProjectPrompts`（plan 5.2 节 SYSTEM 文本约束 5 项：可执行 reference / ≥5 test_cases / KC 匹配 mastered_kcs / 仅 Python 标准库 / 难度匹配）。`CareerMicroProject` record 字段扩为 `id, userId, majorCode, title, briefMd, judgeProblemId, status, score, createdAt, completedAt` 10 项。
- 2026-05-06 **[复用/可见性提升]** `AiProblemTestCaseWriter` 由 package-private 提升为 `public`（`writeTestCases` / `buildTestCaseScoreJson` 同步），让 Career Project Studio 复用现成的 test_case 落盘工具，避免重复实现 OJ runtime 期望的 `info` 元数据格式。原 `SpecializedProblemGenerator` 0 改动。
- 2026-05-06 **[新增/REST]** `CareerStudioController`（`/api/career/studio/*`）4 个端点：`GET /recommendations`（推荐 KC 簇）/ `POST /projects`（生成微项目，失败返回 `error("generation_failed")`）/ `GET /projects?limit=5`（学生最近 N 个项目）/ `GET /projects/{id}`（详情，资源属主校验下沉到 service 层 user_id 双键 SQL，不属于则 404）。所有端点未登录抛 401。
- 2026-05-06 **[新增/单测]** `MicroProjectStudioServiceImplTest`（11 用例）：(1)(2) recommendForUser mastery 全 < 0.5 / 部分 ≥ 0.5；(3) critic 拒绝 ⇒ 不真判题、不落库；(4)(5) reference_solution.code 缺失 / test_cases 空 ⇒ 不真判题；(6) 真判题失败（一组 case WA） ⇒ 不落库 + 不写 test_case 文件；(7) 真判题异常（Judge Server 不可用） ⇒ 不落库；(8) 真判题通过 ⇒ 落 `problem` 表 + `career_micro_project` 双 INSERT、返回 `judge_problem_id`；(9) markCompleted 不存在 ⇒ 404；(10) 正常 ⇒ 写 `PROJECT_COMPLETED` 里程碑；(11) lookup user_id 抛 EmptyResult ⇒ 不重复写里程碑。**未启动 Spring Context**，纯 Mockito ≤ 0.3s。
- 2026-05-06 **[强约束/不绕过 Judge Server]** Studio `generate` 严格走 `LanguagePackProblemJudgeCheckService.executeReferenceSolution`（已存在的判题入口，与 OJ 主判题链路同源 `/judge` 协议），从未引入新的判题路径；落 `problem` 表后学生提交时走标准 `SubmissionService` → `JudgeCompletedEvent` → `JudgeCompletedEventListener` → `LearnerMasteryServiceUnified.updateMastery` → `CareerMilestoneEventListener.onMasteryUpdated`（todo 10）形成完整闭环。`career_micro_project.judge_problem_id` 是普通 FK，与 problem 表对齐 plan 0 节强约束。
- 2026-05-06 **[验证]** `mvn -Dtest='MicroProjectStudioServiceImplTest' test`：11/11 全过；`mvn -Dtest='ReflectionServiceImplTest,CareerBridgingServiceImplTest,DomainLensServiceImplTest,CareerPathServiceImplTest,CareerMilestoneEventListenerTest,MicroProjectStudioServiceImplTest' test`：48/48 全过、0 failure 0 error；`mvn -q compile` + `mvn -q test-compile`：0 错误。
...
- 2026-05-06 **[新增/V84]** `V84__career_bridging.sql`：新建 `career_bridging_milestone`（里程碑触发表，5 类 type：enrollment / kc_cluster_graduated / chapter_entered / project_completed / path_node_unlocked）+ `career_bridging_report`（Why 报告表，含 citations JSONB / rollout_mode / reflection_passed / trace_id）+ 3 个索引。
- 2026-05-06 **[新增/灰度配置]** `AlethicodeProperties` 新增 `Career.Bridging` 内部类（`enabled` 默认 true、`treatmentRate` 默认 0.5），env 前缀 `ALETHICODE_CAREER_BRIDGING_*`。关闭时服务层 fail fast 抛 503，不写兜底降级。
- 2026-05-06 **[新增/服务]** `com.alethicode.service.career.bridging` 包：`CareerBridgingService`（接口 6 方法）+ `CareerBridgingServiceImpl`（注册触发 → A/B 分组 → LLM 生成 → Reflection critic → 写库 → 标记消费完整链路）+ `MilestoneType` 枚举 + `CareerBridgingReport` 投影 record + `CareerBridgingPrompts`（Why 层 system prompt + user prompt 拼装）。LLM 调用走 `AiModelGateway.callForJson`，Reflection 走 `ReflectionService.reflectAndRefine(CardType.CAREER_BRIDGING, ...)`，A/B 走 `RolloutPolicyService.assignAbTest("career_bridging_v1", ...)`。
...
- 2026-05-06 **[新增/services/judge-server]** 上游 `QingdaoU/JudgeServer/server/` 完整 fork 到 `services/judge-server/judge_server/`（8 个 Python 模块 + `unbuffer.c` + `entrypoint.sh`），上游 `QingdaoU/Judger@newnew` 完整 fork 到 `services/judge-server/judger/`（CMakeLists + src/*.c + src/rules/*.c + bindings/Python + LICENSE）。包名由上游裸 `server/` 改为 `judge_server/` 满足 AGENTS.md 的 Python snake_case 命名规范；文件内仍然采用扁平 import，与上游 Docker 镜像运行时布局一致。SATA license 全文随源码保留在 `services/judge-server/judger/LICENSE`，按 license 要求保留原版权声明与项目 URL。
- 2026-05-06 **[新增/工程化壳层]** `services/judge-server/` 新增工程化文件：`pyproject.toml`（项目内依赖管理，`requires-python = ">=3.10"` 兼容 host 烟测、生产镜像内固定 3.12）、`Dockerfile`（基于 `debian:trixie-slim` 多阶段构建：builder 段编译 judger C 内核 + 打 _judger Python wheel；final 段安装 python3.12 / golang / temurin-21-jdk / nodejs / gcc-13 全语言 toolchain，与上游镜像能力对齐）、`Makefile`（`make dev/test/lint/image/compose-up/compose-down/clean`）、`README.md`（含上游 fork 出处、开发模式两种方式、目录结构、路线图）、`.gitignore` / `.dockerignore`、`configs/default.toml`（Phase 1-6 配置键 ops 唯一索引：worker_pool / diagnosis / explain / scheduling / metrics / trace / safety 七组占位，本 phase **一律不消费**）。
- 2026-05-06 **[新增/测试]** `services/judge-server/tests/` 新增 7 个 Phase 0 host 烟测：包元数据、上游 8 个 Python 文件齐全性、judger C 内核构建产物齐全性、`configs/default.toml` 解析与未来 Phase section 全在场、工程化壳层文件齐全性、`pyproject.toml` 元数据最小不变量；1 个 sandbox 标记测试在 host 上自动跳过（真实判题需要 libseccomp + libjudger.so + 三个特殊用户 compiler/code/spj，必须在判题镜像内跑）。host 上 `python3 -m pytest -v` 验证 6 passed / 1 skipped。
- 2026-05-06 **[新增/文档]** `services/judge-server/docs/UPSTREAM.md` 记录两份上游 fork 出处的 commit hash + SATA license + 季度同步 SOP（diff 上游 + cherry-pick 安全补丁 + 更新 fork commit 列）；`services/judge-server/docs/release-notes.md` 按 Phase 顺序登记能力变更摘要，Phase 0 已写实，Phase 1-6 给出预期能力 + 配置开关 + 回退方式占位。
- 2026-05-06 **[新增/部署]** `deploy/docker-compose.judge-only.yml` 新建：开发者按需单独启动本地构建的判题机镜像 `alethicode/judge-server:dev`（端口 12358 与上游 `JudgeServer/docker-compose.example.yml` 对齐；只读根文件系统 + cap_drop + tmpfs + privileged 与生产模板对齐；默认 `DISABLE_HEARTBEAT=1` 避免本地误回报到生产 backend）。`deploy/helm/alethicode/values-judge-local.yaml` 新建：Helm overlay 把 `judge.image.repository` 切到项目镜像，使用方式 `helm upgrade -f values.yaml -f values-judge-local.yaml`。
- 2026-05-06 **[强约束/不破坏现网]** **`deploy/docker-compose.yml` 与 `deploy/helm/alethicode/values.yaml` 默认值未修改**——本地 backend 仍连上游 `registry.cn-hongkong.aliyuncs.com/oj-image/judge:1.6.1`，`mvn spring-boot:run` 启动 backend 完全不依赖 `services/judge-server/` 进程，满足用户「开发模式启动暂时不要用本地判题机」的硬约束。项目镜像 `alethicode/judge-server:dev` 仅在显式 opt-in（compose -f overlay 或 Helm -f overlay）时启用，避免本地 30+ 分钟构建（cmake + libseccomp + apt install python3.12/golang/temurin-21-jdk/nodejs）成本扩散给所有开发者。
- 2026-05-06 **[Phase 0 验证]** 标准回归（"老 Java 后端连接新镜像，提交一组覆盖 AC/WA/TLE/MLE/RE/CE 的标准样本，结果与原镜像 100% 一致"）需要 docker buildx + Linux 沙箱环境，本会话 host 暂不具备完整 sandbox 用户与 cmake + libseccomp 工具链，**先把工程化骨架与 host 烟测落地**；真实镜像构建与回归对比作为 Phase 1 进入前的前置验收项，由后续 sprint 在 docker-enabled 环境补做。host 烟测已经覆盖：包结构齐全 / 配置文件可解析 / 上游源码完整 / 工程化壳层完备四类前置约束。
- 2026-05-06 **[不动文件清单]** 本批 commit 严格遵守「只动 services/judge-server/ + deploy/{docker-compose.judge-only.yml,helm/.../values-judge-local.yaml} + CHANGELOG.md」边界：未修改 `backend/` 任何 Java 源码、未修改 `frontend/` 任何 Vue/JS 源码、未修改 `services/tutor-graph/` 与 `services/alethicode-rag/`、未修改 `contracts/` 任何契约、未修改 `deploy/docker-compose.yml` 与 `deploy/helm/.../values.yaml` 默认 judge 镜像配置；与并发推进的 `career-bridging-closure_e71ce32e.plan.md` 与 `chat composer phase plan_4c10c6a0.plan.md` 路径完全无重叠。
...
- 2026-05-03 **[业务/account]** `AccountServiceImpl.applyResetPassword` 行为彻底重写：依次走 (1) 已登录拦截 (2) captcha 校验 (3) Redis 60 秒按邮箱限流 (4) 找用户——不存在 / `is_disabled=true` 时**不报错**、**不写 token**、**不发邮件**、直接返回 success（不泄露存在性） (5) 写 32 位随机 token 与 `now + 20 min` 过期时间 (6) 同步调 `PasswordResetMailService.sendResetEmail`。原本的 `User does not exist` 错误响应被移除——攻击者无法再通过响应分辨邮箱是否存在。`AccountServiceImpl` 构造函数从 3 参数扩展为 5 参数（新增 `PasswordResetMailService` / `PasswordResetThrottle`），属于 service 内部边界变更，无 deprecated 别名。原有"已成功使用即作废"的 token 单次性由现有 `resetPassword` 在重置时清空 `reset_password_token` 字段保障，本次未触动该路径。
- 2026-05-03 **[领域/account]** 新增 `service/account/PasswordResetMailService` 接口与 `impl/PasswordResetMailServiceImpl`：单一职责负责"读 `sys_options.smtp_config` + 拼模板 + 调 `SmtpMailService.send`"。SMTP 缺 server / port / email / password 任一字段即抛 `BadRequestException("Please setup SMTP config at first")`，与 admin `/api/admin/smtp-test` 文案一致；攻击者看到的是系统级错误，不会借此区分用户存在性。链接 base-url 走 `SystemOptionService.getWebsiteConfig().websiteBaseUrl()`，自带 admin 配置 → properties 默认值的 fallback；邮件正文为纯文本，包含 `${baseUrl}/reset-password/${token}` 与 20 分钟有效期提示。读 `sys_options.smtp_config` 的方式与 `BetaFeedbackMailNotifier` 保持一致（直接 `JdbcTemplate` + `ObjectMapper`），不破坏 `SystemOptionService` 对 password 字段的封装。
- 2026-05-03 **[领域/account]** 新增 `service/account/PasswordResetThrottle` 接口与 `impl/RedisPasswordResetThrottle`：通过 `StringRedisTemplate.opsForValue().setIfAbsent(key, "1", Duration.ofSeconds(60))` 实现 60 秒按邮箱冷却。key 形态 `password_reset_throttle:<lower(email)>`，邮箱在落 key 前先做 `trim().toLowerCase(Locale.ROOT)`，避免大小写绕过。Redis 不可用时异常向上抛，整条申请失败——Redis 是项目既定基础设施（spring-session、限流均依赖），失联即系统级故障，不在此处掩盖。
- 2026-05-03 **[安全/account]** 移除"邮箱不存在 → `User does not exist`"分支。攻击者无法再通过这一响应分辨用户是否存在；限流 key 落在他们提交的 email 字符串上，对存在与不存在的邮箱效果一致，60 秒内同邮箱重复探测一律被拒（`Reset email already sent recently, please try again later`）。
- 2026-05-03 **[测试/account]** 新增 12 个单元用例并先行 RED 验证：`PasswordResetMailServiceImplTest` 3 例（完整配置 → 真发邮件且 subject/body 含 token 链接；缺 password 字段 → BadRequest；smtp_config 整行缺失 → BadRequest）；`RedisPasswordResetThrottleTest` 3 例（首次 setIfAbsent 60s TTL；窗口内第二次返回 false；大小写归一）；`AccountServiceImplApplyResetPasswordTest` 6 例（captcha 错 / 已登录 / 用户存在 → 写 token + 发邮件 / 用户不存在 → 静默 success / 60 秒内重复 → 限流 / SMTP 缺失 → 异常向上传播）。`AccountServiceImpl` 嵌套 record `UserRow` 由 `private` 提升为 package-private，使同包测试能直接构造 stub 行；外部包仍不可见。`AccountServiceImplLogoutTest` 同步补两路构造函数 mock 依赖。`mvn test -Dtest='!*IntegrationTest,!TutorWorkflowControllerTest'` 共计 619 单元/契约测试全过。
- 2026-05-03 **[文档]** `docs/plans/2026-05-03-password-reset-mail-design.md` 沉淀本次设计（行为表 + 组件 + 测试 + 不做事项）。

### `/guide` 学生使用手册产品化重构（默认 Notion / Cursor / Claude 文档风）

> **背景**：原 `/guide` 默认走"奶蛙吉祥物 + 趣味动效主导"，对非 PY 初学者不够专业；同时新发布的 AI 导学助手、@ 上下文引用、课件问答三块核心能力没有专门章节，学生不知道怎么用。本次按 plan `guide-page-product-refactor_46fcb4b4.plan.md` 把页面重做为以学生视角组织的产品化手册：默认极简文档风（白底 + 灰边 + mono kicker + 大留白），把所有奶蛙、贴片、流程手绘版、confetti 收到右上角"打开趣味模式"开关后；新增 3 个核心模块、重写 4 个老模块、删掉 1 个被替代的旧模块，单元 + e2e 测试契约同步更新。

- 2026-05-03 **[内容/manual]** `frontend/src/pages/oj/views/manual/manualContent.js` 大改写：`SECTIONS` 由 9 项改 10 项（新顺序 `welcome / ai / context / qa / flow / tips / faq / tour / gallery / feedback`，删除 `core`，新增 `context` / `qa`）；新增 9 个内容常量（`HERO_CAPABILITIES` / `AI_CAPABILITIES` / `RECOMMENDED_PROMPTS` / `DISCOURAGED_PROMPTS` / `CONTEXT_TOKENS` / `CONTEXT_EXAMPLES` / `CONTEXT_TIPS` / `COURSEWARE_QA_SCOPE` / `COURSEWARE_QA_PROMPTS` / `COURSEWARE_QA_NOTES` / `LEARNING_LOOP_STEPS`）；重写 4 个老常量（`QUICK_START_STEPS` 由 3 步扩到 4 步并配 `where / look / why` 三段；`FLOW_NODES` 改成 8 步学习闭环 `read / io / idea / code / submit / feedback / fix / review`；`TIPS` 改成 5 条对应学习节奏的最佳实践；`FAQ_ITEMS` 8 条按"做题流程顺序"排）；删除 `CORE_OPERATIONS` 和 `QA_GUIDE`（内容拆到 ai / context / qa / flow / tour 各 section）。`@ token` 一律对齐 `frontend/src/pages/oj/views/problem/UnifiedAgentPanel.vue` 的 `SHORTHAND_BY_TYPE`，覆盖代码里实际存在的 `@card:<id>` + 7 个 `@last_*`。
- 2026-05-03 **[视觉/manual]** `sections/shared.less` 默认走 Notion / Cursor / Claude 文档风（白底 `var(--bg-card)` + 1px 灰边 + 40px×44px 大留白 + mono kicker + 26px 标题 + `letter-spacing: -0.4px`），删除原"温暖 warm-primary 边 + shadow-sm"的视觉；section 头部 `ManualNaiwaSticker` 默认 `display: none`，仅在 `ManualPage.vue` 根节点 `.is-fun` 时由 `:deep(.manual-section__head .manual-naiwa-sticker)` 恢复。`ManualSidebar.vue` 默认去掉 `backdrop-filter` 玻璃效果，走纯 `var(--bg-card)` + 灰边；`funMode` prop 默认值由 `true` 改为 `false`，仅 `.is-fun` 时恢复磨砂玻璃。
- 2026-05-03 **[组件/manual]** 新增 `ManualPromptCard.vue`：用于 `SectionAI` / `SectionContext` / `SectionCoursewareQa` 公用的"示例提问"卡，顶部标签 + monospace 主体 + 复制按钮（`navigator.clipboard.writeText` 优先，`document.execCommand('copy')` fallback），不引入新依赖。
- 2026-05-03 **[组件/manual]** 新增 `sections/SectionContext.vue`（`@ 上下文引用`）：4 小节（@ 是什么 + 内联 SVG 弹出菜单示意图 → 什么时候用 → 8 个 token 表 → 4 条示例 PromptCard → 4 条使用建议 callout）。
- 2026-05-03 **[组件/manual]** 新增 `sections/SectionCoursewareQa.vue`（`课件问答`）：4 小节（是什么 + 与 AI 导学助手的区别 → 7 类用法 → 5 个 PromptCard → 4 条注意事项 callout）+ 底部"打开课件问答 →"按钮（`$router.push('/language-pack-qa')`）。
- 2026-05-03 **[重构/manual]** `sections/SectionAI.vue` 从"5 角色 + ai-qa-guide 嵌入"重构为四段式（能做什么 8 卡 → 5 角色手风琴 → 推荐提问 PromptCard → 不推荐提问红色 callout）+ 保留底部"AI 是辅助，不是答案"warning。`QA_GUIDE` 已下线，相关展示拆到 `SectionCoursewareQa`。
- 2026-05-03 **[重构/manual]** `sections/SectionWelcome.vue` 重写为 4 步快速开始，每步用 `where / look / why` 三段元信息呈现，CTA 改为「跟着 8 步走完整流程」+「先读 AI 导学助手说明」。`sections/SectionFlow.vue` 重写为 8 步学习闭环，复用 `ManualFlowDiagram` 渲染节点，节点点击仍跳到对应章节（前 4 步指向 ai、submit 指向 faq、后 3 步分别指 context / context / tips）；流程图下方追加 `LEARNING_LOOP_STEPS` 文字说明。`sections/SectionFAQ.vue` / `SectionTips.vue` / `SectionTour.vue` / `SectionFeedback.vue` 文案与 kicker 编号同步新顺序，`SectionTour` 标题加上"附录"前缀。
- 2026-05-03 **[删除/manual]** 删除 `sections/SectionCore.vue`（5 折叠面板的"核心操作"）：内容已被 ai / context / qa / flow / tour 各 section 接管，强行保留只会产生信息冗余。
- 2026-05-03 **[重构/manual]** `ManualPage.vue` Hero 重写：标题"和 AI 一起，把每道题学透"+ `Alethicode 使用指南` kicker + 副标段 + 4 张能力卡（`HERO_CAPABILITIES` 渲染，点击跳转对应 section）+ 双 CTA（「立刻开始」→ `welcome`、「了解 AI 导学助手」→ `ai`）；`funMode` 默认值由 `true` 改为 `false`，`localStorage(manual.fun_mode) === 'on'` 才打开；hero 吉祥物图、`ManualTypewriter`、`ManualFlowingText` + `ManualStatsCounter` 全部 `v-if="funMode"` 门控；`ManualNaiwaWidget` / `ManualNaiwaMouseFollower` / `ManualNaiwaRandomPopper` / `SectionGallery` 维持原 funMode 双门控。Section 注册从 9 个改 10 个，新增 `SectionContext` / `SectionCoursewareQa`，移除 `SectionCore`。
- 2026-05-03 **[测试/manual]** 同步更新 4 个 spec：`tests/unit/manual-content-data.spec.js` 重写（10 SECTIONS、新顺序、CORE_OPERATIONS / QA_GUIDE 反向断言、新增 11 个常量的存在性 + 数量断言、HERO_CAPABILITIES target 必须是合法 SECTION id）；`tests/unit/manual-page-source.spec.js` 重写（10 section import 顺序、`SectionCore` 反向断言、`HERO_CAPABILITIES` import、funMode 默认 false、`v-if="funMode"` 门控覆盖 typewriter / mascot / hero-stats）；`tests/unit/manual-sections-source.spec.js` 重写（10 section 公共契约 + 新增 `ManualPromptCard` 契约、`SectionAI` 不再引 `QA_GUIDE`、`SectionContext` SVG + role="table" + `ManualPromptCard` 引用、`SectionCoursewareQa` `$router.push('/language-pack-qa')` 按钮、`SectionCore.vue` 已删除断言）；`tests/e2e/manual-guide.spec.js` 删除已废弃的 dark-mode / theme 用例，新增 4 张能力卡 / `@ token` 表 / 课件问答按钮 / PromptCard 复制按钮的可见性断言，趣味模式默认 off 后所有 widget / gallery / follower 的可见性断言反转（先点 toggle 才生效）。
- 2026-05-03 **[lint]** `ManualFlowDiagram.vue` 顺手修一处 `vue/no-use-v-if-with-v-for` pre-existing error（v-if + v-for 同元素），把 v-if 提到外层 `<template>`。
- 2026-05-03 **[验证/manual]** `node_modules/.bin/jest tests/unit/manual-*` 6 套件 296/296 通过；`node_modules/.bin/eslint src/pages/oj/views/manual` 0 errors（剩 1 个与本次重构无关的 pre-existing warning）。
